...
function LinkearOren(){
	...
	//Luz
	u_Lposition2 = gl.getUniformLocation(shaderProgramOrenNayar, 'Lpos');
	//materiales
...
function DibujarOren(tMat,Ob,vao,indexC){
	...
	gl.uniform1f(u_Ro,Ob.getMaterial().getRho());
	gl.uniform1f(u_Sigma,Ob.getMaterial().getSigma());

	gl.uniform3fv(u_Ka2, [Ob.getMaterial().getKaX(),Ob.getMaterial().getKaY(),Ob.getMaterial().getKaZ()]);
	gl.uniform3fv(u_Kd2, [Ob.getMaterial().getKdX(),Ob.getMaterial().getKdY(),Ob.getMaterial().getKdZ()]);
	gl.uniform3fv(u_Ks2, [Ob.getMaterial().getKsX(),Ob.getMaterial().getKsY(),Ob.getMaterial().getKsZ()]);
	gl.bindVertexArray(vao);
	gl.drawElements(gl.TRIANGLES, indexC, gl.UNSIGNED_INT, 0);
	//desconecto el vao y el shader
	gl.bindVertexArray(null);
